...
export class Route {
  ...
  private _createRoute() {
    this.pageElement = document.createElement(PageElementNames[this._path]) as PageElement;

    if (this._attrs) {
      for (const [key, value] of Object.entries(this._attrs)) {
        this.pageElement.setAttribute(key, value);
      }
    }
  }
  ...
  unmountRout() {
    this._rootElement.removeChild(this.pageElement);
  }

  mountRout() {
    this._rootElement.appendChild(this.pageElement);
  }
}
